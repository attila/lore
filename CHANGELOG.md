...
- MCP tool descriptions, CLI output and the plugin skills name the pattern collection the "knowledge
  directory", matching the `knowledge_dir` setting. (#75)
...
- Trace maintenance now only deletes trace data files (`.jsonl` / `.jsonl.gz`) after the retention
  horizon; other files in the trace directory are left alone instead of being swept: applies to both
  `lore trace prune` and the SessionStart-triggered lazy maintenance pass. (#60)
...
- Additive at every user-facing surface; the v3 schema bump is one-way: a v0.1.x binary against a v3
  database requires `lore ingest --force` after re-install.
...
- **Delta ingest**: `lore ingest` only re-indexes changed, added, moved, and deleted files since the
  last-ingested commit (via `git diff --name-status`), eliminating the Ollama round-trip penalty for
  unchanged files.
- **Single-file ingest**: `lore ingest --file <path>` indexes one file without requiring a git
  ...
  `--force` overrides.
- **`.loreignore`**: gitignore-style exclude file in pattern repositories. Filters files during full
  and delta ingest, with reconciliation when the file changes. Supports negation patterns, directory
  globs, and recursive globs via the `ignore` crate.
...
- **Universal patterns**: patterns whose `tags:` frontmatter list contains `universal` get always-on
  injection at SessionStart (full body in a `## Pinned conventions` section) and bypass the
  PreToolUse dedup filter so they re-inject on every relevant tool call. Additive beyond `top_k`;
  relevance gate intact. Closes the always-on discoverability gap for process-level conventions like
  commit messages, push discipline, and branch naming. See `docs/pattern-authoring-guide.md` for the
  new "When to use the universal tag" section.
...
- **Release process**: pushing a `v*` tag to GitHub triggers a workflow
  ...
  ship a release. CI gains a cross-compile smoke job for the same four targets so cross-compile
  breakage surfaces on every PR. New `just release-prep VERSION` recipe rotates the CHANGELOG and
  bumps `Cargo.toml`. Maintainer runbook at [`docs/release-process.md`](docs/release-process.md).
