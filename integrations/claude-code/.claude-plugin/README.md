...
- If an option needs a payload (list of numbers, a corrected list), accept it on the same reply when
  the author prefixes the payload with the letter. For example, `B: 1, 3, 4`. If the letter arrives
  without the payload, prompt once more; never guess a default.
...
- Any numbered list that feeds into a letter choice (menus, edit suggestions, tool-call lists) is
  rendered _above_ the `Options:` block, not mixed into it. The numbered list is the menu; the
