...
Bundles the lore MCP server, lifecycle hooks, and user-invocable skills into a single Codex plugin.
See `../README.md` and `../../README.md` for the broader project overview.

## Skill naming convention

Skills inside this plugin are named by their function alone, with no `lore-` prefix. Codex's plugin
namespace handles provenance through its supported skill/plugin invocation surfaces.

For example, the search skill lives at `skills/search/SKILL.md` and is addressable through Codex's
supported explicit skill invocation UI or syntax. New skills follow the same shape - pick a
function-shaped bare name (`coverage-check`, not `lore-coverage-check`) and let the plugin namespace
