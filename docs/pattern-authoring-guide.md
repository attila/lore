...
needed. But indexing alone does not guarantee effectiveness. A pattern must be both _discoverable_
(the search engine finds it) and _actionable_ (the agent follows it). This guide explains how to
...
Consider two real patterns from a production knowledge directory:
...
action context "committing." All three are terms an agent might use when the pattern should surface.
...
When an agent ran `gh pr edit --body`, the hook extracted terms like "edit" and "body." Neither term
appeared in the pattern. The pattern scored zero despite being directly relevant.
...
(two characters) never reaches the search engine. A pattern about `just ci` must contain longer
...
hook's own query extraction on synthetic tool calls (via the `lore extract-queries` subcommand). It
...
`## Pinned
conventions` section. It also bypasses the `PreToolUse` deduplication filter, so it
re-injects on every relevant tool call instead of being suppressed after the first appearance. The
relevance gate stays intact: the pattern still has to match the agent's tool call to inject.
...
- Universal patterns are exempt from the coverage-check skill's discoverability scoring (they bypass
  the channel coverage-check measures). The report marks them so authors do not pointlessly chase
  low coverage scores on a pattern that always re-injects.
...
`applies_when` continues to fire on every relevant call, exactly as before: the predicate is opt-in.
...
The matcher operates on the raw command string. It never passes through the FTS-cleaning that strips
...
It is suppressed on `Bash ls`, `Bash wc -l`, `Bash grep`, and on any non-Bash tool:
...
infers `{rust}`, an `npm test` Bash command infers `{javascript, typescript}`, and a
`pyproject.toml` edit infers `{python}`. The retrieval pipeline then admits patterns that either
...
- Words in a _heading_ carry the most weight (e.g. `## Rust error handling`)
- Words in _tags_ carry the next-most weight (e.g. `tags: [rust, errors]`)
...
single token. Applying it to any other language would be wrong.
...
**A Rust-specific pattern using prose that lacks the canonical token**: retrieval surfaces it on any
Rust file edit even though the body never says "rust":
...
chunk in a file matches a search query, lore injects _every chunk from that file_ into the agent's
context. This sibling chunk expansion is intentional: it ensures the agent receives the full picture
rather than a fragment. But it has a direct consequence for how you organise your pattern files.
...
A knowledge directory often contains markdown files that are not patterns: a README, a CONTRIBUTING
...
Place a `.loreignore` file at the root of your knowledge directory to exclude them. The syntax is
...
A single file covering unrelated topics. When any section matches, every section is injected. A file
mixing "Error Handling," "YAML Formatting," and "Docker Configuration" pollutes the agent's context
with two irrelevant domains every time one matches.
