...
results and pattern listings are _data_, not diagnostics. Using stderr made the output invisible to
...
The dividing line: if the output is the _purpose_ of running the command (search results, pattern
...
- Any CLI command whose primary purpose is to _return data_ to the caller
- Commands that agents or scripts might invoke programmatically
- Subcommands like `search`, `list`, `show`, `get`, `export`

Not applicable to:

- Commands whose purpose is to _perform an action_ (like `init`, `ingest`) — their progress output
